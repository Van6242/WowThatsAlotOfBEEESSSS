...
class GameScene: SKScene, SKPhysicsContactDelegate
{
    ...
    override func didMove(to view: SKView)
    {
        ...
        paddle.physicsBody?.restitution = 1.03
    ...
    func randomrotation()
    {
        ...
        randrot = Double(CGFloat.random(in: 0...2 * .pi))
//        paddle.zRotation = CGFloat(randrot)
        let rotation = SKAction.rotate(byAngle: CGFloat(randrot), duration: 0.1)
        paddle.run(rotation)
        print("turn ended")
    ...
    func createComputerPaddle()
    {
        ...
        compPaddle.texture = SKTexture.init(imageNamed: "comb")
    ...
    func followBall()
    {
        let move = SKAction.moveTo(x: ball.position.x, duration: 0.01)
        compPaddle.run(move)
    }
